...
#include "Abilities/Projectiles/AOProjectileElemental.h"
...
void UAOProjectileElemental::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UAOProjectileElemental::SpawnProjectile(AAOPlayerBase* InstigatorCharacter)
{
	...
	if (ensureAlways(ProjectileClass))
	{
		...
		if (iCombat)
		{
			...
			FTransform SpawnTM = FTransform(ProjRotation, SocketLocation);
			
 			AAOProjectile* Projectile = GetWorld()->SpawnActorDeferred<AAOProjectile>(ProjectileClass, SpawnTM, GetOwningActorFromActorInfo(),
 			Cast<APawn>(GetOwningActorFromActorInfo()), ESpawnActorCollisionHandlingMethod::AlwaysSpawn);
		
			//TODO GameplayEffectSpec for causing damage
