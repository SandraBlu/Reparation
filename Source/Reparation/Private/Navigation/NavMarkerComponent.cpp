...
UNavMarkerComponent::UNavMarkerComponent()
{
	...
	auto LocationIconFinder = ConstructorHelpers::FObjectFinder<UTexture2D>(TEXT("/Script/Engine.Texture2D'/Reparation/Resources/Icons/T_Marker_Default.T_Marker_Default'"));
	...
	MarkerDomain.AddTag(FRGameplayTags::Get().NavigatorTypes_Compass);
	MarkerDomain.AddTag(FRGameplayTags::Get().NavigatorTypes_Minimap);
	MarkerDomain.AddTag(FRGameplayTags::Get().NavigatorTypes_Worldmap);
...
FNavigationMarkerSettings UNavMarkerComponent::GetMarkerSettings(const FGameplayTag& NavigatorType)
{
	FNavigationMarkerSettings Settings = DefaultMarkerSettings;
	FNavigationMarkerSettings Overrides = CompassOverrideSettings;

	if (NavigatorType == FRGameplayTags::Get().NavigatorTypes_Compass)
	{
		Overrides = CompassOverrideSettings;
	}
	else if(NavigatorType == FRGameplayTags::Get().NavigatorTypes_Worldmap)
	{
		Overrides = WorldMapOverrideSettings;
	}
	else if (NavigatorType == FRGameplayTags::Get().NavigatorTypes_Minimap)
	{
		Overrides = MinimapOverrideSettings;
	}
	else if (NavigatorType == FRGameplayTags::Get().NavigatorTypes_Screenspace)
	{
...
void UNavMarkerComponent::SetDomain(const FGameplayTagContainer& InMarkerDomain)
{
...
void UNavMarkerComponent::AddDomains(const FGameplayTagContainer& NewMarkerDomains)
{
